...
@app.post("/workouts/new_workouts/",status_code=status.HTTP_201_CREATED)
async def new_workout(db:db_dependency,newworkout:WorkoutRequest):
    ...
    db.commit()
@app.get("/workouts/workout_by_id/{workout_id}",status_code=status.HTTP_200_OK)
...
@app.put("/workouts/alter_workouts/{workout_id}/",status_code=status.HTTP_204_NO_CONTENT)
async def alter_workout(db:db_dependency,workout_id:int,workout:WorkoutRequest):
    ...
    db.add(workout_model)
    db.commit()
@app.delete("/workouts/delete_workout_by_id/{workout_id}",status_code=status.HTTP_204_NO_CONTENT)
